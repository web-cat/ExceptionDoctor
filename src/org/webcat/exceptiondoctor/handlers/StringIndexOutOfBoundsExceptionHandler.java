		...
		AbstractExceptionHandler implements ExceptionHandlerInterface
{
			...
			SourceCodeHiddenException
	{
		...
		/*
		 * if (variables.size() > 0) { if (variables.size() == 1) { newMessage
		 * += "It appears that the code was trying to access " +
		 * "an element at index " + index + " of the String \"" +
		 * variables.get(0) + "\".  "; } else { newMessage +=
		 * "It appears that the code was trying to access " +
		 * "an element at index " + index +
		 * " of one of the Strings in the line of code: ("; // print the
		 * variable/String names for (int i = 0; i < variables.size(); i++) {
		 * newMessage += "\"" + variables.get(i) + "\""; if (i <
		 * variables.size() - 1) { newMessage += " or "; } } newMessage +=
		 * ").  "; } } else { newMessage +=
		 * "It appears that the code was trying to access an " +
		 * "element at index " + index + " of a String.  "; } // show an error
		 * message if it's a negative index if (Integer.parseInt(index) < 0) {
		 * newMessage += "Remember, you cannot have a negative index for " +
		 * "accessing String characters."; } else { newMessage +=
		 * "The size of the String may be less than " + index +
